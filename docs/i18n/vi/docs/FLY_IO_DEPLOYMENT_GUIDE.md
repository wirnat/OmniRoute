...
Ứng dụng OmniRoute của Fly.io là một trong những ứng dụng được yêu thích nhất hiện nay:
...
- 平台: Fly.io
- 部署方式:本地 `flyctl` 直接发布
- 运行方式：使用仓库内现有 `Dockerfile` và `fly.toml`
...
Bạn có thể sử dụng Flyctl`二进制并放到`PATH` 中。### 3.2 登录 Fly 账号
...
Bạn có thể làm được điều đó không?```toml
...
Fly.io là một trong những ứng dụng được yêu thích nhất trên Fly.io.### 5.1 已验证使用的参数
...
Tôi có thể sử dụng `INITIAL_PASSWORD`, giúp bạn có được một mật khẩu mới.
...
| 变量名                   | 是否推荐 | 说明                           |
| ------------------------ | -------- | ------------------------------ | ---------------------- |
| `API_KEY_BÍ MẬT`         | 必需     | Khóa API 生成与校验使用        |
| `JWT_BÍ MẬT`             | 必需     | 登录态和 JWT 签名使用          |
...
- `DATA_DIR=/data` 非常关键，必须与 Khối lượng bay 挂载点一致
- `NEXT_PUBLIC_BASE_URL` 用于调度器和前端回调等场景---

## 7. 一键设置参数

Một trong những điều thú vị nhất là Fly Secrets.

说明:

- 不包含 `INITIAL_PASSWORD`
- 适用于当前项目 `toàn tuyến````powershell
...
如果当前仓库是 ngã ba, 并且你要同步上游 `https://github.com/diegosouzapw/OmniRoute` 的更新,推荐按下面流程执行。
...
- cái nĩa `origin` 指向你自己的
- `ngược dòng` 指向原仓库

如果没有 `ngược dòng`,先添加:```powershell
...
- `git merge ngược dòng/chính` 用于同步原仓库最新代码
- `gitcheck HEAD~1 -- fly.toml` 用于恢复合并前你 fork 自己的 `fly.toml`
- 如果上游没有改 `fly.toml`,这一步不会带来额外差异
- 如果上游改了 `fly.toml`,这一步能确保 Fly 应用名、挂载卷、区域等 fork 自定义部署配置不被覆盖

Bạn có thể sử dụng `v3.4.7``,也可以先确认标签是否已经包含在 `upstream/main`:```powershell
...
1. `git tìm nạp ngược dòng --tags`
2. `git merge ngược dòng/chính`
3. 恢复 fork 的 `fly.toml`
4. `git đẩy nguồn gốc chính`
5. `triển khai flyctl`
6. `trạng thái flyctl -một đường dẫn`
7. `flyctl log --no-tail -a omniroute`
...
- 你还没执行 `bộ bí mật flyctl`
- 你打开的是另一个应用,例如 `định tuyến`, 不是 `toàn tuyến`### 12.2 `flyctl deploy` 报 `app not found`
...
1. `đăng nhập xác thực flyctl`
2. `ứng dụng flyctl tạo ra mọi tuyến đường`
3. `bộ bí mật flyctl ... -a omniroute`
4. `triển khai flyctl`
5. `flyctl log --no-tail -a omniroute`
