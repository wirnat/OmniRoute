...
如果安装脚本在当前环境失败，也可以手动下载 `flyctl` 二进制并放到 㸭PATH`### 3.2 登录 Fly 账号
...
建议放入 Secretele zborului:

| 变量名                   | 是否推荐 | 说明                           |
| ------------------------ | -------- | ------------------------------ | ---------------------- |
| `API_KEY_SECRET`         | 必需     | Cheie API 生成与校验使用       |
| `JWT_SECRET`             | 必需     | 登录态和 JWT 签名使用          |
| `STORAGE_ENCRYPTION_KEY` | 强烈推荐 | 加密存储敏感连接信息           |
| `MACHINE_ID_SALT`        | 推荐     | 生成稳定机器标识               |
| `PAROLA_INIȚIALĂ`        | 可选     | 首次部署时直接指定后台初始密码 |
...
如果当前仓库是 furk，并且你要同步上游 `https://github.com/diegosouzapw/OmniRoute` 的更新，推荐新，推荐按浉下荐按按果当前仓库是
...
如果你想合并上游最新 `main`，并强制保留 furcă 当前的 `fly.toml`，可按下面流稌：执衋：执```powershell
...
- `git checkout HEAD~1 -- fly.toml` 用于恢复合并前你 furk 自己的 `fly.toml`
- 如果上游没有改 `fly.toml`，这一步不会带来额外差异
- 如果上游改了 `fly.toml`，这一步能确保 Fly 应用名、挂载卷、区域等 furcă自定义部署配置不被覆盖

如果你明确只想对齐某个发布标签，例如 `v3.4.7`，也可以先确认标签先确认标签是廫否孾是廥`amonte/principal`：```powershell
...
1. `flyctl auth login`
2. „aplicațiile flyctl creează omniroute”.
