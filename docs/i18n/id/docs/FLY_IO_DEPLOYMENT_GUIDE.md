...
Bagaimana cara menggunakan OmniRoute dan Fly.io?

- 首次把当前项目部署到 Fly.io
- 后续代码更新后继续发布
- 新项目参考同样流程部署

rute perjalanan `omniroute`。---

## 1. 部署目标

- 平台：Fly.io
- 部署方式：本地 `flyctl` 直接发布
- Penggantian `Dockerfile` dan `fly.toml`
...
`fly.toml`, 重点看这一行：```toml
app = 'omniroute'

````

如果准备部署到自己的新应用，可改成全局唯一名称：```toml
...
前项目没有设置 `INITIAL_PASSWORD`,因为本次部署按需求不使用它。
...
建议放入 Rahasia Terbang：

| 变量名                   | 是否推荐 | 说明                           |
| ------------------------ | -------- | ------------------------------ | ---------------------- |
| `API_KEY_SECRET`         | 必需     | Kunci API                      |
| `JWT_RAHASIA`            | 必需     | 登录态和 JWT                   |
...
- `DATA_DIR=/data` 非常关键,必须与 Volume Terbang 挂载点一致
...
如果控制台 `Rahasia` 页面没有显示你期待的变量，先检查：
...
如果你明确只想对齐某个发布标签,例如 `v3.4.7`,也可以先确认标签是否已经包含在`hulu/utama`：```powershell
...
1. `git mengambil hulu --tags`
2. `git menggabungkan hulu/utama`
3. 恢复 garpu 的 `fly.toml`
4. `git push asal utama`
5. `penyebaran flyctl`
...
`/app/data/...`, yang berarti `DATA_DIR`, 需要立即修正。---
...
1. 修改 `fly.toml` 里的 `aplikasi`
2. melalui `NEXT_PUBLIC_BASE_URL`
3. Pilih `DATA_DIR=/data`
...
1. `login autentikasi flyctl`
2. `aplikasi flyctl membuat omniroute`
3. `rahasia flyctl disetel ... -a omniroute`
4. `penerapan flyctl`
