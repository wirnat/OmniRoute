...
| Пункт     | Мінімальна                  | Рекомендовано    |
| --------- | --------------------------- | ---------------- |
| **ЦП**    | 1 vCPU                      | 2 vCPU           |
| **RAM**   | 1 ГБ                        | 2 ГБ             |
...
- Зверніть увагу на**публічну IP**(наприклад, `203.0.113.10`)### 1.2 Connect via SSH
...
> **Порада**: для максимальної безпеки обмежте порти 80 і 443 лише IP-адресами Cloudflare. Перегляньте розділ [Розширений захист](#advanced-security).---
...
> ⚠️**ВАЖЛИВО**: генеруйте унікальні секретні ключі! Використовуйте `openssl rand -hex 32` для кожного ключа.### 2.3 Start the container
...
Має відображатися: `База даних SQLite готова` і `слухає порт 20128`.---
...
4. Скопіюйте**Сертифікат походження**та**Закритий ключ**```bash
...
Тримайте тайм-аути потоку зворотного проксі-сервера у відповідності зі змінними env тайм-ауту OmniRoute. Якщо підняти
`FETCH_TIMEOUT_MS` / `STREAM_IDLE_TIMEOUT_MS`, підняти `proxy_read_timeout` / `proxy_send_timeout`
вище того самого порогу.### 3.3 Enable and Test
...
| --- | ------ | --------------------------------------------- | --------- | --------------------- |
| A   | `llms` | `203.0.113.10` (IP-адреса віртуальної машини) | ✅ Проксі | ### 4.2 Configure SSL |
...
- Автоматичне перезапис HTTPS: ✅ Увімкнено### 4.3 Testing
...
Додайте наступне до `nginx.conf` всередині блоку `http {}`:```nginx
...
Для віддаленого доступу через Cloudflare Workers (без безпосереднього доступу до віртуальної машини):```bash
...
Перегляньте повну документацію на [omnirouteCloud/README.md](../omnirouteCloud/README.md).---
