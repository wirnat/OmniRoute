...
- Quelle: Fly.io
- 部署方式: `flyctl` 直接发布
- Erhalten Sie Zugriff auf die Datei „Dockerfile“ und „fly.toml“.
- 数据持久化: Fly Volume wird in „/data“ gespeichert
- Weitere Informationen: „https://omniroute.fly.dev/“.---
...
Sobald Sie `flyctl` und die `PATH`-Funktion aktiviert haben, können Sie auch `flyctl` verwenden.### 3.2 登录 Fly 账号
...
- Klicken Sie auf die Schaltfläche „fly.toml“ und „app“.
...
Die Fly.io-App ist kostenlos verfügbar.### 5.1 已验证使用的参数
...
Weitere Informationen zu Fly Secrets:

| 变量名                       | 是否推荐 | 说明                           |
| ---------------------------- | -------- | ------------------------------ | ---------------------- |
| `API_KEY_SECRET`             | 必需     | API-Schlüssel 生成与校验使用   |
...
| OAuth/API-Benutzeroberfläche | 按需     | 各类外部平台鉴权配置           | ### 6.2 当前项目推荐值 |
...
- Geben Sie „INITIAL_PASSWORD“ ein
...
- `git checkout HEAD~1 -- fly.toml` Erweitern Sie den Fork um `fly.toml`
...
3. Öffnen Sie den Fork von „fly.toml“.
...
Geben Sie die Datei „/app/data/...“ ein und verwenden Sie „DATA_DIR“.---
...
- `fly.toml` löscht `destination = '/data'`
- `DATA_DIR` ist eine Datei mit `/data`### 12.5 不设置 `INITIAL_PASSWORD` 是否能跑
...
1. Öffnen Sie „fly.toml“ und „app“.
2. Geben Sie „NEXT_PUBLIC_BASE_URL“ ein
3. Geben Sie „DATA_DIR=/data“ ein
4. Öffnen Sie „API_KEY_SECRET“, „JWT_SECRET“, „MACHINE_ID_SALT“, „STORAGE_ENCRYPTION_KEY“.
5. Klicken Sie auf „/data“.
...
1. „flyctl Auth Login“.
2. „Flyctl-Apps erstellen Omniroute“.
