...
- 数据持久化: Repülési hangerő 挂载到 `/data`
...
建议放入 Repülési titkok:

| 变量名                   | 是否推荐 | 说明                           |
| ------------------------ | -------- | ------------------------------ | ---------------------- |
| "API_KEY_SECRET"         | 必需     | API kulcs 生成与校验使用       |
...
如果当前仓库是 fork，并且你要同步上游 `https://github.com/diegosouzapw/OmniRoute` 的更新，推併荐按
...
如果你明确只想对齐某个发布标签，例如 `v3.4.7`, "felfelé/fő":```powershell
...
1. `flyctl auth login`
2. "flyctl alkalmazások omniroute-t hoznak létre".
