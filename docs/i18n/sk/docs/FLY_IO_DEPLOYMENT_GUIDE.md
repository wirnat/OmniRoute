...
如果安装脚本在当前环境失败，也可以手动下载 `flyctl` 二进制并放到 `PATH` 二进制并放到 `PATH` 也可以手动下载 `flyctl`### 3.2 登录 Fly 账号
...
当前项目没有设置 `ÚVODNÉ_HESLO`，因为本次部署按需求不使用它。
...
如果控制台 `Tajomstvá` 页面没有显示你期待的变量，先检查：
...
- `pôvod` 指向你自己的 vidlica
- „proti prúdu“ 指向原仓库

如果没有 `proti prúdu`，先添加：```powershell
...
如果你想合并上游最新 `hlavný`，并强制保留 vidlica 当前的 `fly.toml`，可按下面觚程扉```powershell
...
- 如果上游改了 `fly.toml`，这一步能确保 Fly 应用名、挂载卷、区域等 vidlica自定义部署配置不被覆盖

如果你明确只想对齐某个发布标签，例如 `v3.4.7`，也可以先确认标吷孾是否孾是否孾是否孾是否吷孾是否吷孾是否签`upstream/main`:```powershell
...
1. `flyctl auth login`
2. „aplikácie flyctl vytvárajú omniroute“.
