...
- `KLUCZ_PAMIĘCI_ENCRYPTION_### 5.2 关于 `INITIAL_PASSWORD`
...
建议放入 Sekrety latania:

| 变量名                    | 是否推荐 | wiadomość                |
| ------------------------- | -------- | ------------------------ | ---------------------- |
| `API_KEY_SECRET`          | 必需     | Klucz API 生成与校验使用 |
| `JWT_SECRET`              | 必需     | Zarządzanie zasobami JWT |
| `KLUCZ_PAMIĘCI_ENCRYPCJI` | 强烈推荐 | Pokaż wszystko           |
| `ID_MASZYNY_SALT`         | 推荐     | 生成稳定机器标识         |
| `INITIAL_HASŁO`           | 可选     | Szkło hartowane          |
| Rozwiązanie OAuth/API     | 按需     | 各类外部平台鉴权配置     | ### 6.2 当前项目推荐值 |
...
如果控制台 `Sekrety` 页面没有显示你期待的变量,先检查:
...
如果当前仓库是 widelec, 并且你要同步上游 `https://github.com/diegosouzapw/OmniRoute`的更新,推荐按下面流程执行.
...
- 如果上游改了 `fly.toml`, 这一步能确保 Fly 应用名、挂载卷、区域等 widelec自定义部署配置不被覆盖

如果你明确只想对齐某个发布标签,例如 `v3.4.7`,也可以先确认标签是否已经包含在`poprzedni/główny`:```powershell
...
1. `pobierz git z góry --tags`
2. `git merge upstream/main`
3. 恢复 widelec 的 `fly.toml`
4. `git push Origin main`
5. `wdrożenie flyctl`
6. `status flyctl - omniroute`
7. `logi flyctl --no-tail -a omniroute`
...
- `DATA_DIR` 是否设置为 `/dane`### 12.5 不设置 `INITIAL_PASSWORD` 是否能跑
...
1. 修改 `fly.toml` 里的 `aplikacja`
2. Otwórz `NEXT_PUBLIC_BASE_URL`
3. Otwórz `DATA_DIR=/data`
...
1. `logowanie uwierzytelniające flyctl`
2. `Aplikacje flyctl tworzą omniroute`
3. `zestaw sekretów flyctl... -omniroute`
4. `wdrożenie flyctl`
5. `logi flyctl --no-tail -a omniroute`
