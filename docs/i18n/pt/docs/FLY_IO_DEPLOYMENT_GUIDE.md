...
- Nome: Fly.io
- 部署方式:本地 `flyctl` 直接发布
- 运行方式: 使用仓库内现有 `Dockerfile` e `fly.toml`
...
- `DADOS_DIR`
- `JWT_SECRET`
- `MACHINE_ID_SALT` -`NEXT_PUBLIC_BASE_URL`
...
| 变量名                   | 是否推荐 | Tradução                              |
| ------------------------ | -------- | ------------------------------------- | ---------------------- |
| `API_KEY_SECRET`         | 必需     | Chave API 生成与校验使用              |
| `JWT_SECRET`             | 必需     | Para obter mais informações sobre JWT |
| `STORAGE_ENCRYPTION_KEY` | 强烈推荐 | Produtos de limpeza                   |
| `MACHINE_ID_SALT`        | 推荐     | 生成稳定机器标识                      |
| `INITIAL_PASSWORD`       | 可选     | 首次部署时直接定后台初始密码          |
| Aplicativos OAuth/API    | 按需     | 各类外部平台鉴权配置                  | ### 6.2 当前项目推荐值 |
...
如果控制台 `Segredos` 页面没有显示你期待的变量，先检查：

- 看的应用是不是 `omniroute`
- `fly.toml` e `app` 是否和控制台应用一致---
...
1. `git buscar upstream --tags`
2. `git merge upstream/main`
3. Use fork em `fly.toml`
4. `git push origem principal`
5. `implantação flyctl`
...
- 你还没执行 `conjunto de segredos flyctl`
...
1. Use `fly.toml` e `app`
2. Use `NEXT_PUBLIC_BASE_URL`
3. Use `DATA_DIR=/data`
4. Use `API_KEY_SECRET`,`JWT_SECRET`,`MACHINE_ID_SALT`,`STORAGE_ENCRYPTION_KEY`
...
1. `login de autenticação flyctl`
2. `aplicativos flyctl criam omniroute`
3. `conjunto de segredos flyctl ... -a omniroute`
4. `implantação flyctl`
5. `logs flyctl --no-tail -a omniroute`
