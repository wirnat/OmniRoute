...
本文档记录了 OmniRoute 在 Fly.io 上的实际部署方法，适用于两类场景：

- 首先将当前项目部署到Fly.io
- 后续代码更新后续发布继续
- 新项目参考同样流程部署

这里基于当前项目已经验证通过的配置整理，应用名为`omniroute`。---

## 1. 部署目标

- 平台：Fly.io
- 部署方式：本地 `flyctl` 直接发布
- 运行方式：使用仓库现有 `Dockerfile` 和 `fly.toml`
...
- 本项目必须设置 `DATA_DIR=/data`，否则数据库和密钥会写入容器临时目录---
...
如果安装脚本在当前环境失败，也可以手动下载`flyctl`二进制文件并放到`PATH`中。### 3.2 登录 Fly 账号
...
`fly.toml`，重点看这一行打开：```toml
app = 'omniroute'

````

如果您准备部署自己的新应用程序，可以更改全局唯一名称，例如：```toml
app = 'omniroute-yourname'
````

注意：

- 控制台里不是与 `fly.toml` 里 `app` 一致的应用
- 以前如果用过名字之外，例如 `oroute`，不要和 `omniroute` 混淆### 4.3 创建应用
...
如果您已经改成其他应用程序名称，请将 `omniroute` 替换成您的名字。### 4.4 首次部署
...
当前项目没有设置`INITIAL_PASSWORD`，因为本次配置按需求不使用它。
...
| 指标名                   | 是否推荐                | 说明                           |
| ------------------------ | ----------------------- | ------------------------------ |
| `API_KEY_SECRET`         | 简单                    | API Key 生成与校验使用         |
| `JWT_SECRET`             | 简单                    | 登录状态和 JWT 签名使用        |
| `STORAGE_ENCRYPTION_KEY` | 强烈推荐                | 加密存储敏感连接信息           |
| `MACHINE_ID_SALT`        | 推荐                    | 生成稳定机器标识               |
| `初始密码`               | 任选                    | 首次部署时直接指定后台初始密码 |
| OAuth/API 依赖           | ｜ 大众外部平台鉴权配置 | ### 6.2 当前项目推荐值         |
...
- `NEXT_PUBLIC_BASE_URL` 用于调度器和回调回调等场景---
...
如果控制台`Secrets`页面没有显示您期待的变量，请先检查：
...
如果只更新参数，不修改代码：```powershell
flyctl secrets set KEY=value -a omniroute
````

Fly会自动滚动更新机器。### 9.1 跟踪原仓库更新并保留 fork 的 `fly.toml`

如果当前仓库是fork，并且你要同步上游 `https://github.com/diegosouzapw/OmniRoute` 的更新，推荐下面按流程执行。
...
如果你想合并上游最新的`main`，并强制保留fork当前的`fly.toml`，可按下面流程执行：```powershell
...
- `git merge upper/main` 用于同步原仓库最新代码
- `git checkout HEAD~1 -- Fly.toml` 用于恢复合并前你 fork 自己的 `fly.toml`
- 如果上游没有修改 `fly.toml`，这一步不会带来额外的差异
- 如果上游改了 `fly.toml`，这一步可以保证 Fly 应用名、挂载卷、区域等 fork 自定义部署配置不被覆盖

如果你明确匹配某个发布标签，例如`v3.4.7`，也可以先确认标签是否已经包含在`upstream/main`中：```powershell
...
1.`git fetch 上游 --tags` 2.`git 合并上游/主` 3. 恢复 fork 的 `fly.toml` 4. `git push origin main` 5.`flyctl部署` 6. `flyctl status -a 全方位路径` 7.`flyctl 日志 --no-tail -a 全方位路径`
...
- `/data/server.env` 说明运行时按键达到了持久卷
- `/data/storage.sqlite` 说明 数据库写入持久卷

如果你看到的是`/app/data/...`，说明`DATA_DIR`没有安装，需要立即修改。---
...
- 你还没有执行 `flyctl Secrets set`
- 你打开的是另一个应用程序，例如“oroute”，而不是“omniroute”### 12.2 `flyctl deploy` 报 `app not found`
...
修改可以运行，但会退回到默认`CHANGEME`。生产环境建议加快后台密码。---

## 13. 新项目复用建议

如果以后是新项目照着一份文档部署，至少修改这几项：

1.修改`fly.toml`里的`app` 2.修改`NEXT_PUBLIC_BASE_URL` 3. 保留`DATA_DIR=/data` 4.重新生成`API_KEY_SECRET`、`JWT_SECRET`、`MACHINE_ID_SALT`、`STORAGE_ENCRYPTION_KEY` 5. 首次部署后检查日志是否写入`/data`
...
1.`flyctl 身份验证登录` 2.`flyctl 应用程序创建全向` 3.`flyctl 秘密设置...-a 全方位路径` 4.`flyctl部署` 5.`flyctl 日志 --no-tail -a 全方位路径`
