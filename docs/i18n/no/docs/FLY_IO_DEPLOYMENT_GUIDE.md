...
| 变量名                   | 是否推荐 | 说明                           |
| ------------------------ | -------- | ------------------------------ | ---------------------- |
| `API_KEY_SECRET`         | 必需     | API-nøkkel 生成与校验使用      |
...
- `opprinnelse` 指向你自己的 gaffel
- `oppstrøms` 指向原仓库

如果没有 `oppstrøm`，先添加：```powershell
...
如果你明确只想对齐某个发布标签，例如 `v3.4.7`，也可以先确认标签竐圌`oppstrøms/hoved`:```powershell
git merge-base --is-ancestor v3.4.7 upstream/main
````

返回成功表示 `oppstrøms/hoved` 已经包含该版本，直接合并 `oppstrøms/hoved` 即可。### 9.2 同步上游后的标准发布顺序
...
7. `flyctl logger --no-tail -a omniroute`
...
1. `flyctl auth login`
2. `flyctl-apper oppretter omniroute`
3. `flyctl hemmeligheter satt ... -en omniroute`
4. `flyctl deploy`
5. `flyctl logger --no-tail -a omniroute`
