...
- Prendi nota dell'**IP pubblico**(ad esempio, `203.0.113.10`)### 1.2 Connect via SSH
...
> **Suggerimento**: per la massima sicurezza, limita le porte 80 e 443 solo agli IP Cloudflare. Consulta la sezione [Sicurezza avanzata](#advanced-security).---
...
> ⚠️**IMPORTANTE**: genera chiavi segrete uniche! Utilizza `openssl rand -hex 32` per ciascuna chiave.### 2.3 Start the container
...
Dovrebbe essere visualizzato: "[DB] Database SQLite pronto" e "in ascolto sulla porta 20128".---
...
4. Copia il**Certificato di Origine**e la**Chiave Privata**```bash
...
Mantieni i timeout del flusso del proxy inverso allineati con le variabili di ambiente timeout OmniRoute. Se rilanci
`FETCH_TIMEOUT_MS` / `STREAM_IDLE_TIMEOUT_MS`, raise `proxy_read_timeout` / `proxy_send_timeout`
sopra la stessa soglia.### 3.3 Enable and Test
...
| -------- | ------ | ------------------------------------------- | ---------- | --------------------- |
| A        | `llms` | `203.0.113.10` (IP della macchina virtuale) | ✅ Procura | ### 4.2 Configure SSL |
...
- Riscritture HTTPS automatiche: ✅ On### 4.3 Testing
...
Aggiungi quanto segue a `nginx.conf` all'interno del blocco `http {}`:```nginx
...
Per l'accesso remoto tramite Cloudflare Workers (senza esporre direttamente la VM):```bash
