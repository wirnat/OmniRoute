...
本文档记录 OmniRoute su Fly.io 上的实际部署方法, 适用于两类场景：
...
- Nome: Fly.io
- 部署方式：本地 `flyctl` 直接发布
- Modificare il file `Dockerfile` e `fly.toml`
...
Per favore, usa "flyctl" per "PATH" e `PATH`.### 3.2 登录 Fly 账号
...
Per saperne di più, 可改成全局唯一名称, 例如:```toml
...
| 变量名                   | 是否推荐 | 说明                           |
| ------------------------ | -------- | ------------------------------ | ---------------------- |
| `API_KEY_SECRET`         | 必需     | Chiave API 生成与校验使用      |
| `JWT_SECRET`             | 必需     | 登录态e JWT 签名使用           |
| `STORAGE_ENCRYPTION_KEY` | 强烈推荐 | 加密存储敏感连接信息           |
| "ID_MACCHINA_SALE"       | 推荐     | 生成稳定机器标识               |
| `PASSWORD_INIZIALE`      | 可选     | 首次部署时直接指定后台初始密码 |
...
- `NEXT_PUBLIC_BASE_URL` 用于调度和前端回调等场景---

## 7. 一键设置参数

Scopri di più su Fly Secrets.
...
如果控制台 "Segreti" 页面没有显示你期待的变量, 先检查：
...
Vola 会自动滚动更新机器.### 9.1 跟踪原仓库更新并保留 fork 的 `fly.toml`
...
- `git checkout HEAD~1 -- fly.toml` Fork del fork `fly.toml`
- 如果上游没有改 `fly.toml`, 这一步不会带来额外差异
- 如果上游改了 `fly.toml`, 这一步能确保 Fly 应用名、挂载卷、区域等 fork 自定义部署配置不被覆盖

Per favore, usa "v3.4.7" per "upstream/main":```powershell
...
5. "distribuzione flyctl".
6. `flyctl status -a omniroute`
7. `flyctl logs --no-tail -a omniroute`

È possibile utilizzare la versione `v3.4.7` per la versione 3.4.7.---
...
Puoi usare `/app/data/...`, quindi `DATA_DIR`.---
...
- "DATA_DIR" è "/data".### 12.5 不设置 `INITIAL_PASSWORD` 是否能跑
...
1. Utilizzare `fly.toml` come `app`
2. Utilizzare `NEXT_PUBLIC_BASE_URL`
3. Scegliere `DATA_DIR=/data`
...
1. `login autenticazione flyctl`
2. "Le app flyctl creano omniroute".
3. `set di segreti flyctl ... -a omniroute`
4. "distribuzione flyctl".
