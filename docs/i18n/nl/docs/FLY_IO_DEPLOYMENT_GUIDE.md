...
Gebruik OmniRoute en Fly.io voor meer informatie:

- 首次把当前项目部署到 Fly.io
- 后续代码更新后继续发布
- 新项目参考同样流程部署

当前项目已经验证通过的配置整理,应用名为 `omniroute`。---

## 1. 部署目标

- Naam: Fly.io
- 部署方式:本地`flyctl`直接发布
- 运行方式:使用仓库内现有 `Dockerfile` en `fly.toml`
- 数据持久化: Vliegvolume 挂载到 `/data`
...
- `app = 'omniroute'` 决定实际部署到哪个 Vliegen 应用
- `bestemming = '/data'` 决定持久卷挂载目录
...
'flyctl' 二进制并放到 'PATH' 中。### 3.2 登录 Fly 账号
...
Meer informatie over Fly.io is beschikbaar via Fly.io.### 5.1 已验证使用的参数
...
- `OPSLAG_ENCRYPTION_KEY`### 5.2 关于 `INITIAL_PASSWORD`

当前项目没有设置 `INITIAL_PASSWORD`,因为本次部署按需求不使用它。

如果不设置:

- 启动日志会提示默认密码是`WIJZIGING`
- 部署后应尽快在系统设置中修改登录密码

如果希望无人值守初始化后台密码,也可以后续补:
...
| 变量名                   | 是否推荐 | 说明                             |
| ------------------------ | -------- | -------------------------------- | ---------------------- |
| `API_KEY_SECRET`         | 必需     | API-sleutel 生成与校验使用       |
| `JWT_SECRET`             | 必需     | JWT-producten voor JWT-producten |
| `STORAGE_ENCRYPTION_KEY` | 强烈推荐 | Overige producten                |
| `MACHINE_ID_SALT`        | 推荐     | 生成稳定机器标识                 |
| `INITIAL_PASSWORD`       | 可选     | 首次部署时直接指定后台初始密码   |
| OAuth/API-software       | 按需     | 各外部平台鉴权配置               | ### 6.2 当前项目推荐值 |
...
- `DATA_DIR=/data` 非常关键,必须与 Vliegvolume 挂载点一致
- `NEXT_PUBLIC_BASE_URL` 用于调度器和前端回调等景---
...
Vliegen### 9.1 跟踪原仓库更新并保留 fork 的 `fly.toml`

如果当前仓库是 vork, 并且你要同步上游 `https://github.com/diegosouzapw/OmniRoute` 的更新，推荐按下面流程执行。
...
- 'oorsprong' 指向你自己的 vork
- `stroomopwaarts` 指向原仓库
...
如果你想合并上游最新 `main`,并强制保留 vork 当前的 `fly.toml`,可按下面流程执行:```powershell
...
- 如果上游改了 `fly.toml`,这一步能确保 Fly 应用名、挂载卷、区域等 vork自定义部署配置不被覆盖

如果你明确只想对齐某个发布标签,例如 `v3.4.7`,也可以先确认标签是否已经包含在`stroomopwaarts/hoofd`:```powershell
...
3. 恢复 vork 的 `fly.toml`
4. `git push origin main`
5. `flyctl implementeren`
6. `flyctl-status - een omniroute`
...
1. Gebruik `fly.toml` of `app`
2. Klik op `NEXT_PUBLIC_BASE_URL`
3. Selecteer `DATA_DIR=/data`
...
1. `flyctl auth-aanmelding`
2. `flyctl-apps creëren omniroute`
3. `flyctl geheimen set... -een omniroute`
4. `flyctl implementeren`
