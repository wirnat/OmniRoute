...
- Noteer het**openbare IP**(bijvoorbeeld `203.0.113.10`)### 1.2 Connect via SSH
...
> **Tip**: Voor maximale veiligheid beperkt u poort 80 en 443 alleen tot Cloudflare IP's. Zie de sectie [Geavanceerde beveiliging](#advanced-security).---
...
> ⚠️**BELANGRIJK**: Genereer unieke geheime sleutels! Gebruik `openssl rand -hex 32` voor elke sleutel.### 2.3 Start the container
...
Het zou moeten weergeven: `[DB] SQLite database gereed` en `luisteren op poort 20128`.---
...
4. Kopieer het**Oorsprongscertificaat**en de**Privésleutel**```bash
...
Houd reverse-proxy-streamtime-outs afgestemd op uw OmniRoute-time-out-env-vars. Als je verhoogt
`FETCH_TIMEOUT_MS` / `STREAM_IDLE_TIMEOUT_MS`, verhogen `proxy_read_timeout` / `proxy_send_timeout`
boven dezelfde drempel.### 3.3 Enable and Test
...
| --- | ------ | ---------------------- | ---------------- | --------------------- |
| Een | `llms` | `203.0.113.10` (VM-IP) | ✅ Gevolmachtigd | ### 4.2 Configure SSL |
...
- Automatische HTTPS-herschrijvingen: ✅ Aan### 4.3 Testing
...
Voeg het volgende toe aan `nginx.conf` in het `http {}` blok:```nginx
...
Voor externe toegang via Cloudflare Workers (zonder de VM rechtstreeks bloot te leggen):```bash
...
Zie de volledige documentatie op [omnirouteCloud/README.md](../omnirouteCloud/README.md).---
