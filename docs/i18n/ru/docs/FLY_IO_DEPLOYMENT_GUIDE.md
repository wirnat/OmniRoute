...
Доступ к OmniRoute и Fly.io 上的实际部署方法,适用于两类场景:
...
- Приложение: Fly.io
- 部署方式:本地 `flyctl` 直接发布
- 运行方式:使用仓库内现有 `Dockerfile` и `fly.toml`
- 数据持久化: Fly Volume 挂载到 `/data`
- 访问地址:`https://omniroute.fly.dev/`---

## 2. 当前项目关键配置

Используйте файл `fly.toml` для загрузки файлов:```toml
app = 'omniroute'
primary_region = 'sin'
...
Приложение Fly.io 上建议至少配置以下参数。### 5.1 已验证使用的参数
...
| 变量名                   | 是否推荐 | 说明                           |
| ------------------------ | -------- | ------------------------------ | ---------------------- |
| `API_KEY_SECRET`         | 必需     | Ключ API 生成与校验使用        |
...
| Поддержка OAuth/API      | 按需     | 各类外部平台鉴权配置           | ### 6.2 当前项目推荐值 |
...
- 适用于当前项目 `всенаправленный маршрут````powershell
...
Песня «Секреты» в исполнении «Секреты»:

- 看的应用是不是 `всенаправленный маршрут`
- `fly.toml` и `app` 是否和控制台应用一致---
...
- вилка `origin` 指向你自己的
- `вверх по течению` 指向原仓库

如果没有 «вверх по течению», 先添加:```powershell
...
如果你明确只想对齐某个发布标签, 例如 `v3.4.7`, 也可以先确认标签是否已经包含在`восходящий/основной`:```powershell
...
3. 恢复 вилка 的 `fly.toml`
4. `git push origin main`
5. `flyctl развертывание`
6. `flyctl status - всенаправленный маршрут`
...
如果你看到的是 `/app/data/...`, `DATA_DIR` 没配对,需要立即修正。---
...
- `DATA_DIR` или `/data`### 12.5 不设置 `INITIAL_PASSWORD` 是否能跑
...
2. Введите `NEXT_PUBLIC_BASE_URL`
3. Введите `DATA_DIR=/data`
4. Введите `API_KEY_SECRET`, `JWT_SECRET`,`MACHINE_ID_SALT`,`STORAGE_ENCRYPTION_KEY`
...
1. `авторизация Flyctl `
2. «Приложения Flyctl создают омнирут»
3. `flyctl secrets set... -a omniroute`
4. `flyctl развертывание`
