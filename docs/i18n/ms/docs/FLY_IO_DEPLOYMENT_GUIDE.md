...
- 平台：Fly.io
- 部署方式：本地 `flyctl` 直接发布
- 运行方式：使用仓库内现有 `Fail Docker` 和 `fly.toml`
- 数据持久化：Volume Terbang 挂载到 `/data`
...
- `app = 'omniroute'` 决定实际部署到哪个 Terbang 应用
...
这些参数已经在当前 `laluan omni` 应用上实际部署：

- `RAHSIA_KUNCI_API`
- `DATA_DIR`
- `JWT_SECRET`
- `MACHINE_ID_SALT`
- `URL_ASA_AWAM_SETERUSNYA`
...
| 变量名                   | 是否推荐 | 说明                           |
| ------------------------ | -------- | ------------------------------ | ---------------------- |
| `RAHSIA_KUNCI_API`       | 必需     | Kunci API 生成与校验使用       |
| `JWT_RAHSIA`             | 必需     | 登录态和 JWT 签名使用          |
| `STORAGE_ENCRYPTION_KEY` | 强烈推荐 | 加密存储敏感连接信息           |
| `MACHINE_ID_SALT`        | 推荐     | 生成稳定机器标识               |
| `KATA_laluan_AWAL`       | 可选     | 首次部署时直接指定后台初始密码 |
| OAuth/API 私密凭证       | 按需     | 各类外部平台鉴权配置           | ### 6.2 当前项目推荐值 |

| 变量名                    | 推荐值                      |
| ------------------------- | --------------------------- |
| `DATA_DIR`                | `/data`                     |
| `URL_ASA_AWAM_SETERUSNYA` | `https://omniroute.fly.dev` |

说明：

- `DATA_DIR=/data` 非常关键，必须与 Jumlah Lalat 挂载点一致
- `URL_ASA_AWAM_SETERUSNYA` 用于调度器和前端回调等场景---

## 7. 一键设置参数

下面命令会生成安全随机值，并把当前项目需要的参数一次性写入 Rahsia Terbang。

说明：

- 不包含 `AWAL_KATAlaluan`
- 适用于当前项目 `laluan omni````powershell
...
Terbang 会自动滚动更新机器。### 9.1 跟踪原仓库更新并保留 fork 的 `fly.toml`

如果当前仓库是 fork，并且你要同步上游 `https://github.com/diegosouzapw/OmniRoute` 的更新，推荐按下猢按下猢按下猢按下猢。
...
- 如果上游改了 `fly.toml`，这一步能确保 Fly 应用名、挂载卷、区域等 garpu自定义部署配置不被覆盖

如果你明确只想对齐某个发布标签，例如 `v3.4.7`，也可以先确认标签是否已签是否已現含否```powershell
git merge-base --is-ancestor v3.4.7 upstream/main
````

返回成功表示 `hulu/utama` 已经包含该版本，直接合并 `hulu/utama` 即可。### 9.2 同步上游后的标准发布顺序

同步原仓库完成后，推荐按下面顺序发布：

1. `git fetch upstream --tags`
2. `git merge upstream/main`
3. 恢复 garpu 的 `fly.toml`
4. `git push origin main`
5. `flyctl deploy`
6. `status flyctl -a omniroute`
...
2. 修改 `URL_ASA_AWAM_SETERUSNYA`
3. 保持 `DATA_DIR=/data`
4. 重新生成 `RAHSIA_KEY_API`、`RAHSIA_JWT`、`GARAM_ID_MESIN`、`KUNCI_PENYIMPANAN_PENYIMPANAN`
...
1. `log masuk auth flyctl`
2. `apl flyctl mencipta omniroute`
3. `set rahsia flyctl ... -a omniroute`
4. `flyctl deploy`
