...
本文档记录 OmniRoute ใน Fly.io 上的实际部署方法，适用于两类场景：

- 首次把当前项目部署到 Fly.io
- 后续代码更新后继续发布
- 新项目参考同样流程部署

本文基于当前项目已经验证通过的配置整理，应用名为 `ทุกเส้นทาง`。---

## 1. 部署目标

- 平台：Fly.io
- 部署方式：本地 `flyctl` 直接发布
- ลิงก์: 使用仓库内现有 `Dockerfile` และ `fly.toml`
...
- `app = 'omniroute'` 决定实际部署到哪个 บิน 应用
...
วินโดวส์ PowerShell：```powershell
...
ขยาย `fly.toml`, 重点看这一行：```toml
app = 'omniroute'

````

如果คุณ准备部署到自己的新应用，可改成全局唯一名称，例如：```toml
...
| 变量名                   | 是否推荐 | 说明                           |
| ------------------------ | -------- | ------------------------------ | ------------------------------------------ |
| `API_KEY_SECRET`         | 必需     | API Key การเรียนรู้            |
| `JWT_SECRET`             | 必需     | 登录态和 JWT 签名使用          | แชร์                                       |
| `STORAGE_ENCRYPTION_KEY` | 强烈推荐 | 加密存储敏感连接信息           |
| `MACHINE_ID_SALT`        | 推荐     | 生成稳定机器标识               |
| `INITIAL_PASSWORD`       | 可选     | 首次部署时直接指定后台初始密码 |
| OAuth/API 私密凭证       | 按需     | 各类外部平台鉴权配置           | 各类外部平台鉴权配置### 6.2 当前项目推荐值 |
...
下的命令会生成安全随机值，并把当前项目需要的参数一次性写入 Fly Secrets。

说明：

- และ `INITIAL_PASSWORD`
- 适用于当前项目 `ทุกเส้นทาง````powershell
...
如果控制台 `Secrets` 页เลดี้没有显示คุณ期待的变量，先检查：

- 看的应用是不是 `ทุกเส้นทาง`
- `fly.toml` ของ `app` 是否和控制台应用一致---
...
如果当前仓库是 fork，并且你要同步上游 `https://github.com/diegosouzapw/OmniRoute` 的更新，推荐按下 Face流程执行。
...
- `ต้นกำเนิด` 指向คุณ自己的ส้อม
- `ต้นน้ำ` 指向原仓库

如果没有 `ต้นน้ำ`，先添加：```powershell
...
如果你想合并上游最新 `main`，并强制保留 fork 当前的 `fly.toml`，可按下的的流程执行：```powershell
...
- `git checkout HEAD~1 -- fly.toml` 用于恢复合并前 คุณ fork 自己的 `fly.toml`
- 如果上游没有改 `fly.toml`, 这一步不会带来额外差异
- 如果上游改了 `fly.toml`，这一步能确保 Fly 应用名、挂载卷、区域等 fork 自定义部署配置不被覆盖

如果คุณ明确只想对齐某个发布标签，例如 `v3.4.7`，也可以先确认标签是否已经包含在 `upstream/main`：```powershell
git merge-base --is-ancestor v3.4.7 upstream/main
````

返回成功表示 `ต้นน้ำ/หลัก` 已经包含该版本，直接合并 `ต้นน้ำ/หลัก` 即可。### 9.2 同步上游后的标准发布顺序

同步原仓库完成后，推荐按下 Face顺序发布：

1. `git ดึงข้อมูลอัปสตรีม --tags`
2. `git รวมอัพสตรีม/หลัก`
3. 恢复 fork ของ `fly.toml`
4. `git push origin main`
5. `ปรับใช้ flyctl`
6. `สถานะ flyctl - ทุกสิ่งรอบตัว`
7. `บันทึก flyctl --no-tail -a omniroute`
...
- คุณ还没执行 `ชุดความลับของ flyctl`
- คุณกำลังเริ่มต้นระบบ 是另一个应用，例如 `oroute`，不是 `omniroute`### 12.2 `flyctl deploy` 报 `app not found`
...
- `DATA_DIR` คำสั่ง `/data`### 12.5 不设置 `INITIAL_PASSWORD` 是否能跑

可以运行，但会回退到默认 `CHANGEME`。生产环境建议尽快修改后台密码。---

## 13. 新项目复用建议

如果以后是新项目foto着这份文档部署，最少改这几项：

1. ใช้ `fly.toml` และ `app`
2. ถัดไป `NEXT_PUBLIC_BASE_URL`
3. จากนั้น `DATA_DIR=/data`
4. `API_KEY_SECRET`、`JWT_SECRET`、`MACHINE_ID_SALT`、`STORAGE_ENCRYPTION_KEY``
...
如果只是正常发版，核heart就是：```powershell
flyctl deploy
````

如果是新环境首次部署，核heart就是：

1. `เข้าสู่ระบบรับรองความถูกต้อง flyctl`
2. `แอป flyctl สร้างทุกเส้นทาง`
3. `ชุดความลับของ flyctl ... -ทุกเส้นทาง`
4. `ปรับใช้ flyctl`
5. `บันทึก flyctl --no-tail -a omniroute`
